...
class ExportPage(webapp.RequestHandler):
	...
	def get(self):
		...
		xml_string = '<?xml version="1.0" encoding="UTF-8"?><worldCrises xmlns:xsi="http://www.w3.org/2001  \
			/XMLSchema-instance"xsi:noNamespaceSchemaLocation="wc.xsd">'
		...
		org_info = ['info_type', 'info_history']
		org_contact = ['info_contact_phone', 'info_contact_email']
		org_mail = ['info_contact_mail_address', 'info_contact_mail_city', \
			'info_contact_mail_state', 'info_contact_mail_country', 'info_contact_mail_zip']
        
		for org in orgs:
			xml_string += "<organization id=\"" + getattr(org, 'orgid') + "\">"
			xml_string += "<name>" + str(getattr(org, "name")) + "</name>"
			xml_string += "<info>"
			for info in org_info:
				strdata = info.lstrip("info_", "")
				if getattr(org, info):
					xml_string += "<" + strdata + ">" + str(getattr(org, field)) + "</" + strdata + ">"
				else:
					xml_string += ("<" + strdata + "></" + strdata + ">")
			xml_string += "<contact>"
			for contact in org_contact:
				strdata = info.lstrip("info_contact_", "")
				if getattr(org, contact):
					xml_string += "<" + strdata + ">" + str(getattr(org, field)) + "</" + strdata + ">"
				else:
					xml_string += ("<" + strdata + "></" + strdata + ">")
			xml_string += "<mail>"
			for mail in org_mail:
				strdata = info.replace("info_contact_mail_", "")
				if getattr(org, mail):
					xml_string += "<" + strdata + ">" + str(getattr(org, field)) + "</" + strdata + ">"
				else:
					xml_string += ("<" + strdata + "></" + strdata + ">")
			xml_string += "</mail></contact></info>"
            
			xml_string += "<misc>" + str(getattr(org, "misc")) + "</misc>"
			xml_string += "<ref>" + str(getattr(org, "links")) + "</ref>"
			xml_string += "</organization>"
            ...
            for org in orgs:
                if org.find('.//info'):
                    ...
                    o = Organization(orgid = org.attrib['id'],
                                     ...
                                     info_contact_phone = contact.find('.//phone').text,
                                     info_contact_email = contact.find('.//email').text,
                                     info_contact_mail_address = mail.find('.//address').text,
                                     info_contact_mail_city = mail.find('.//city').text,
                                     info_contact_mail_state = mail.find('.//state').text,
                                     info_contact_mail_country = mail.find('.//country').text,
                                     info_contact_mail_zip = mail.find('.//zip').text,
...
class Organization(db.Model):
    ...
    info_type = db.StringProperty()
    info_history = db.TextProperty()
    info_contact_phone = db.StringProperty()
    info_contact_email = db.StringProperty()
    info_contact_mail_address = db.StringProperty()
    info_contact_mail_city = db.StringProperty()
    info_contact_mail_state = db.StringProperty()
    info_contact_mail_country = db.StringProperty()
    info_contact_mail_zip = db.StringProperty()
